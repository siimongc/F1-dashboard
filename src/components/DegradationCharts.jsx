...
// ── Chart 2: Desgaste acumulado — eje continuo dividido por stints ───────────

function StintDegradation() {
  const keys = Object.keys(stintsData)
  const [selected, setSelected] = useState('Bahrain_Grand_Prix')
  ...
  const { chartData, stintMeta, pitLaps, mae, r2, yDomain, worstStint, validStints } = useMemo(() => {
    ...
    // Build flat array keyed by race_lap; one column per stint
    const byLap = {}
    ...
    // Stint metadata for background areas & pit markers
    ...
    // Pit lap = gap between end of one stint and start of next
  ...
  return (
    <div>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 4, padding: '28px 20px 8px 4px' }}>
        <ResponsiveContainer width="100%" height={300}>
          <ComposedChart data={chartData} margin={{ top: 24, right: 24, left: 4, bottom: 20 }}>
            ...
            {/* Pit stop zones — banda roja visible entre stints */}
            {stintMeta.slice(0, -1).map((s, i) => {
              const nextStart = stintMeta[i + 1].startLap
              const x1 = s.endLap - 0.5
              const x2 = nextStart + 0.5
              return (
                <ReferenceArea
                  key={`pit-${i}`}
                  x1={x1} x2={x2}
            ...
            {/* One line per stint, colored by compound */}
            {validStints.map(s => (
              <Line
                key={s.stint}
                type="monotone"
      ...
      <InsightPill color={C.green}>
        El modelo aprende esta curva de desgaste por compuesto. Con R²=<b>{r2?.toFixed(4)}</b>, predice cuándo el neumático empieza a perder grip y cuánto tiempo acumula antes del pit stop.
      </InsightPill>
    </div>
  )
}

// ── Chart 3: MAE por circuito ────────────────────────────────────────────────
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 4, padding: '16px 16px 8px 8px' }}>
        <ResponsiveContainer width="100%" height={380}>
          ...
          >
            ...
            <XAxis
              type="number"
              domain={[0, Math.max(...data.map(d => d.mae)) * 1.15]}
              tick={axisStyle} axisLine={false} tickLine={false}
              tickFormatter={v => `${v.toFixed(1)}s`}
              label={{ value: 'Error por vuelta (segundos)', position: 'insideBottom', offset: -4,
                fill: C.gray, fontSize: 9, fontFamily: "'Titillium Web', sans-serif" }}
            />
            <YAxis
            ...
            />
            <ReferenceLine
              x={+avgMae} stroke={C.yellow} strokeDasharray="5 3" strokeWidth={1.5}
            ...
            <Bar dataKey="mae" radius={[0, 3, 3, 0]} barSize={20} isAnimationActive animationDuration={600}>
              ...
              {data.map(d => (
                <Cell
                  key={d.id}
                  fill={d.mae < 0.65 ? C.green : d.mae < 1.0 ? C.orange : C.red}
                  opacity={0.85}
                />
              ))}
            </Bar>
